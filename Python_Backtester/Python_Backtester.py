...
class Backtester():
    def __init__(self, stock, funds, target, Days, Increment, FillAtOpen, endDateTime, paper=True):
        ...
        self.crossover = False # maybe move this elsewhere
        self.paper = paper
        self.funds = funds
        self.api = IBapi()
    ...
    def run(self):
        ...
        self.api.connect('127.0.0.1', 7496, 123)
        ...
        for index, row in self.df.iterrows():
            if row['same_day'] == True:
                ...
                for index, row in self.df_by_date.iterrows():
                    ...
                    if crossover == False:
                        ...
                        if row['Close'] > row['VWAP'] and self.position == False:
                            ...
                            self.out_data.append([row['DateTime'],'Buy', row['Close'], 0, 0, row['VWAP']])
            
            
                        elif row['Close'] >= self.target*self.buy_price and self.position == True: 
                            self.position = False
                            crossover = False
                            self.cum_change = self.cum_change + (row['Close']-self.buy_price)/self.buy_price
                            self.out_data.append([row['DateTime'], 'Target' , row['Close'], 1+(row['Close']-self.buy_price)/self.buy_price, self.cum_change, row['VWAP']])
                            #break

                        elif row['Close'] < row['VWAP'] and self.position == True:
                            self.position = False
                            self.cum_change = self.cum_change + (row['Close']-self.buy_price)/self.buy_price
                            self.out_data.append([row['DateTime'], 'Sell' , row['Close'], 1+(row['Close']-self.buy_price)/self.buy_price, self.cum_change, row['VWAP']])
                ...
                time.sleep(.05)
                self.out_df.to_csv('backtest_long.csv')
                self.df_by_date = []
                self.n_days += 1
...
# Enter (stock, quantity, target, Days, Increment, Fill Upon Open, endDateTime)
bot = Backtester('AERC', 100, 1.05, '4 D', '5 mins', True, '20220624 15:59:59')
('AUVI', 800, 1.1, '15 mins', True) 

#bot = Backtester('SIGA', 100, 1.07, '2 D', '5 mins', '20220520 15:59:59')
